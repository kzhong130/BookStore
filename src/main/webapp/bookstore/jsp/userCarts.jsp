...
<a id="btn" href="CartPro!UserInit" class="easyui-linkbutton" data-options="iconCls:'icon-search'">View the shopping carts</a>
	...
	<div id="toolbar2">
		...
		<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-edit" plain="true" onclick="purchase()">Purchase</a>
	</div>
...
<div >
	...
	function purchase() {  
        ...
        for (var i = 0; i < rows.length; i++) {  
            if (strCartNos == '') {  
                strCartNos = rows[i].id;  
            } else {  
                strCartNos += ',' + rows[i].id;  
            }  
        }  
		//document.write(strCartNos);

		
		$.messager.confirm('Confirm','Are you sure you want to purchase these items?',function(r){
			
			if (r){		
				$.post("PurchasePro",{strCartNos:strCartNos},function(result){
					if (result.success){
